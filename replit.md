...
-   **`process-lakecounty-images.js`**: Extracts image filenames from Lake County parsed PDF JSON files. Reads `species.images` and `seedGroups[].images` arrays. Outputs 1 column: Lake County Images (comma-separated list of image filenames). Depends on lakecounty-cache.
